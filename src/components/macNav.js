...
const MacNav = ({ size, label = null, icon = null, className = null }) => {
    return (
        <div className={`descriptionbar-container ${className} text-white px-2 rounded-t-xl rounded-b-none items-center justify-between py-1 flex h-8 w-full font-bold`}>
            <span className="flex">
            ...
            <span className="d-flex align-items-center">{label}{icon}</span>
            <span></span>
        </div>
    )
}
